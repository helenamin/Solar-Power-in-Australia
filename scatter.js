...
function makeResponsive() {
    ...
    // READ CSV FROM FILE
    d3.csv("assets/data/income_scatter.csv").then(function(incomeD) {

      d3.csv("assets/data/rebate_scatter.csv").then(function(rebateData) {     
        
        // Change the key name for postcode in income data to lower case to allow merge
        var incomeData = incomeD.map(item => {
          return { postcode: item.Postcode, Average_total: item.Average_total};
        });

        // convert the income data to numerical
        incomeData.forEach(function(data) {
          data.postcode = +data.postcode;
          data.Average_total = +data.Average_total;
        });

        // convert the rebate data to numerical
        rebateData.forEach(function(data) {
          data.postcode = +data.postcode;
          data.Install_total = +data.Install_total;
          data.Output_total = +data.Output_total;
          data.rebate = +data.rebate;
          });
        
        // Merge the rebate and income data
